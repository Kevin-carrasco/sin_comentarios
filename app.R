...
server <- function(input, output, session) {
  ...
  #   B: decomodifica -> neoliberal (Percepción de servicios públicos y privados)
  #   C: deserve-1 -> deserve-4     (Merecimiento)
  ...
  sd_skip_if(
    ...
    # Entry: route straight from cbc_q6_page to the assigned module's first
    # page in a single hop, so the other two modules are never rendered.
    # Gated on input$cbc_q6 (the slider always carries a default value once
    # that page is reached) so the rule cannot fire earlier than
    # cbc_q6_page - every respondent must still go through the full cbc
    # conjoint task first. sd_is_answered("cbc_q6") is NOT used here: it
    # treats "cbc_q6_comment" as a sub-question of "cbc_q6" (prefix match),
    # which would wrongly require the optional comment box to be filled too.
    # assigned_module == "A" needs no rule: cbc_q6_page's default page_next
    # already points to "clasicos".
    assigned_module == "B" & !is.null(input$cbc_q6) ~ "decomodifica",
    assigned_module == "C" & !is.null(input$cbc_q6) ~ "deserve-1",

    # Exit: once the assigned module's last question is answered, route
    # straight to the closing pages, skipping the other two modules.
    # Module C needs no rule: deserve-4's default page_next already points
    # to "id-final".
    assigned_module == "A" & sd_is_answered("valor_5")  ~ "id-final",
    assigned_module == "B" & sd_is_answered("neolib_4") ~ "id-final"
  )
